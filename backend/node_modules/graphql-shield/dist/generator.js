...
function applyRuleToType(type, rules, options) {
    if (utils_1.isRuleFunction(rules)) {
        ...
        const middleware = Object.keys(fieldMap).reduce((middleware, field) => {
            return Object.assign(Object.assign({}, middleware), { [field]: generateFieldMiddlewareFromRule(rules, options) });
        }, {});
        return middleware;
    }
        ...
        /* Generation */
        const middleware = Object.keys(fieldMap).reduce((middleware, field) => (Object.assign(Object.assign({}, middleware), { [field]: generateFieldMiddlewareFromRule(utils_1.withDefault(defaultTypeRule || options.fallbackRule)(rules[field]), options) })), {});
        return middleware;
    }
    else {
        /* Apply fallbackRule to type with no defined rule */
        const fieldMap = type.getFields();
        const middleware = Object.keys(fieldMap).reduce((middleware, field) => (Object.assign(Object.assign({}, middleware), { [field]: generateFieldMiddlewareFromRule(options.fallbackRule, options) })), {});
        return middleware;
    }
}
/**
...
function applyRuleToSchema(schema, rule, options) {
    ...
    const middleware = Object.keys(typeMap)
        ...
        if (graphql_1.isObjectType(type)) {
            return Object.assign(Object.assign({}, middleware), { [typeName]: applyRuleToType(type, rule, options) });
        }
        else {
            return middleware;
        }
...
function generateMiddlewareFromSchemaAndRuleTree(schema, rules, options) {
    if (utils_1.isRuleFunction(rules)) {
        ...
        const middleware = Object.keys(typeMap)
            ...
            const type = typeMap[typeName];
            if (graphql_1.isObjectType(type)) {
                return Object.assign(Object.assign({}, middleware), { [typeName]: applyRuleToType(type, rules[typeName], options) });
            }
            else {
                return middleware;
            }
